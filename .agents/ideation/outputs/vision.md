...
`vision.md` may move from `draft` to `candidate` when every required section contains substantive content and the resulting vision satisfies the following quality conditions:
...
`vision.md` may move from `candidate` to `approved` only through explicit human approval.
